...
export const initMap = async (k, objectConfig, pathToMapPng, pathToMapJson) => {
  ...
  for (const layer of layers) {
    ...
    const isInteractionObject = objectConfig.interactionObjects.includes(layer.name);

    if (isStaticObject || isInteractionObject) {
      const isStaticProp = layer?.properties?.filter(prop => prop.name === 'isStatic');
      const isStatic = isStaticProp.length > 0 ? isStaticProp[0].value : true;
